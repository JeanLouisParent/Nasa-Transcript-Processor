...
class ImageProcessor:
    ...
    def _deskew(self, image: np.ndarray) -> tuple[np.ndarray, float]:
        ...
        Uses text line analysis for precise skew detection.
        Corrects even very small angles for clean output.

        Args:
            image: Grayscale image

        Returns:
            Tuple of (corrected_image, detected_angle_in_degrees)
        """
        ...
        min_threshold = 0.1
        if abs(median_angle) < min_threshold:
    ...
    def _enhance_light(self, image: np.ndarray) -> np.ndarray:
        """
        Apply light, conservative enhancement.

        This is a gentle enhancement that:
        - Cleans small noise/specks
        - Improves contrast
        - Whitens background
    ...
    def _remove_small_spots(self, image: np.ndarray, max_spot_size: int = 15) -> np.ndarray:
        """
        Remove small isolated dark spots while preserving text.

        Uses connected component analysis to identify and remove
        small dark regions that are likely noise, not text.

        Args:
            image: Grayscale image
            max_spot_size: Maximum area (pixels) to consider as noise
